...
- [ ] Step 7b — scripture lines: `citizen_converted` + `citizen_despair` for both gods (singular + `_many` plural cells), `hunger_pang.maw`, `lower.maw`; tests G3/G4. *(Co-write with Matthew, same process as the 2026-06-04 pass; voice reference is `rhetoric_generation_brief.md`. Off the critical path - step 8 needs only 7a.)*
...
- [ ] **Low-population demographic collapse.** Versus and mirror-match runs: the Maw at the 0.75 spawn peaks at 11–17 citizens and dies of *age with no births* while the centre faction grows to 100+. Two attractor discs 20 tiles apart with nine citizens between them puts nobody within `repro_radius` 2 of a mate. Proposed: scale `attract_probability` by population (`× clamp01(pop / ref)`, ref ≈ 40) — "a village of ten doesn't send pilgrims". Also check the 0.75 spawn site itself (there is a lake at x≈158–176 on seed 42; passive-mode Maw grows fine, so the versus difference is the Open Eye's 240 casts landing on a village of ten).
