...
- [ ] **Maw scripture is placeholder and now on screen.** `rhetoric.json` has no `maw` block for `lower` (the `hunger_pang` gap closed itself when the power was cut, 2026-09-15), so the log shows the literal `<lower>`; conversion cells are placeholders for both gods. Belongs to 7b's gap-fill (co-write session).
- [x] ~~Step 6 — relic intents (place/move/retrieve, push-point targeting) + tests F~~ — built 2026-09-02, 246/246 pass. Added public `RelicManager.can_place` / `can_move` dry runs; amended spec §8 so RELIC_PLACE keys off the push point (agreed with Matthew); added a RELIC_MOVE deadband after measuring 157 relic acts of thrash per round.
- [ ] **Zealot balance — a default round has no middle outcome.** Three seeds at 600 sim_s: player 160 v 26, then **0** v 44, then **0** v 26. The rival either plants three relics forward and converts the player out of existence, or overextends, loses all three to the shatter rule and withers. Dials: `relic_forward_bias` 0.65, relic `amplitude` 20.0, `retrieve_panic` 0.75. Step 8's balance pass, but flagged loudly — it is the first thing a playtest will hit.
- [x] ~~Step 7a — scripture machinery: conversion event channel out of `CitizenManager`, coalescer (leading-edge line, then one `{count}` batch per window), singular/plural cell pick, rival relic verbs voiced into the log behind `[rival] relic_scripture`, tests G1/G2/G5 + channel tests~~ — built 2026-09-11, 255/255 pass. `PowerSystem.voice` is now the one scripture append site.
- [ ] Step 7b — scripture lines: `citizen_converted` + `citizen_despair` for both gods (singular + `_many` plural cells), `lower.maw`; tests G3/G4. *(`hunger_pang.maw` dropped with the power, 2026-09-15 - five cells now, not six.)* *(Co-write with Matthew, same process as the 2026-06-04 pass; voice reference is `rhetoric_generation_brief.md`. Off the critical path - step 8 needs only 7a.)*
- [~] Step 8 — **harness, knobs and `attract_probability` landed 2026-09-11; balance pass held at a design gap.** `densitas/harness.py` (`python -m densitas.harness`), step-6 constants → `[rival]`, `attract_probability` 0.4 → 0.15 (self-starvation; three seeds of evidence). Acceptance at the committed config: 4 of 5 criteria pass; **converts ≥ 5 fails on every run** because the two civilisations never meet. Five findings in the worklog.
- [x] ~~**PR4 contact mechanism — decision.**~~ Chain built 2026-09-11 (step 8b): `RivalAI.push_reach`, `[rival] relic_step_tiles` / `relic_standoff_tiles`. Temple suicide gone — flags forward, at the line, 0 shattered every seed and mode. Acceptance now: places ≥ 2 PASS, casts PASS, pool PASS; converts and relic-threat still FAIL, for the two reasons below.
- [x] ~~**Low-population demographic collapse.**~~ Fixed 2026-09-11 (step 8c): `[powers.relic] attract_pop_ref = 40` scales the pull with alive population. Versus went from Maw-extinct on two seeds (peak 11–17) to 176–192 on every seed; mirror match 106 v 190 / 108 v 147 / 122 v 149. No extinction, starvation or shattered flag in nine rounds.
- [ ] **Relic-threat criterion is structurally blocked** while amplitude is 20 and the standoff is 12: the Maw halts 12 tiles from the player's relics and its bled belief never reaches 1.5× a relic's 20. Resolves with the amplitude item; not a targeting issue.
- [ ] **Relic amplitude 20 vs PR4's population scale.** Three placed relics outweigh a faction of 100. The right lever once contact works; it changes the player's PR3 relic feel, so flagged not changed.
- [ ] **Conversion cascade has no damping** — 36 conversions and an extinction inside 150 s when contact comes early against a weak opponent. `[citizen.faith] drain_rate` is the dial; wants a human on the other side before it moves.
- [x] ~~`rhetoric_generation_brief.md` Maw cast list missing Hunger Pang~~ — fixed 2026-09-11 with 7a.
...
- [x] ~~Rival faction citizens spawn from rival starting point.~~ — PR4 step 3: default-on, `[rival] spawn_frac_x/y`.
