...
- [ ] **PR4 contact mechanism — decision.** The Zealot marches its relics into the enemy temple (`relic_forward_bias` 0.65 lerps toward the enemy *centroid*) where they shatter in seconds; lowering the bias collides with the spread gate and it plants one flag and stops. Proposed: chain each relic off the front-most placed one, a fixed step forward, capped short of enemy relics. Alternatives: closer `spawn_frac_x`; a wander radius that grows with population. Matthew's call, then the acceptance re-runs.
