...
- [ ] Step 7a — scripture machinery: conversion event channel out of `CitizenManager`, coalescer (leading-edge line, then one `{count}` batch per window), singular/plural cell pick, rival relic verbs voiced into the log behind `[rival] relic_scripture`, tests G1/G2/G5 + channel tests. *(Split 2026-09-11; see spec §13 amendment.)*
...
- [ ] Step 8 — Zealot balance pass + AI-vs-AI harness (Steward Open Eye vs Zealot Maw, alongside the passive-player run) + step-6 constants → `[rival]` config + acceptance run (world seed 42 × `ai_seed` 0/1/2) + WORKLOG/TODO/README sync. Difficulty wiring already landed in step 4.
- [ ] `rhetoric_generation_brief.md` lists the Maw's casts as "Plague, Lower Terrain, Comet, Hurricane. Never Bless." Hunger Pang is missing, and it is now the Maw's most-cast power. One line, before 7b.
